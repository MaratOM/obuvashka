<?php
/*****************************************************\
| Sypex Dumper               version 2.0.3 Beta                                     |
|  http://sypex-dumper.ru     sypex@sypex-dumper.ru                         |
|--------------------------------------------------------------------|
|     created: 2003.09.02 19:07              modified: 2009.11.09 19:14    |
|--------------------------------------------------------------------|
| Sypex Dumper is released under the terms of the GNU license             |
...
class Sypex_Dumper {
	function Sypex_Dumper() {
		...
		$this->name = 'Sypex Dumper 2.0.3 Beta';
	}
	...
	function init(){
		...
		if (!ini_get('safe_mode') && function_exists('set_time_limit')) set_time_limit($CFG['time_web']);
		$this->CFG = &$CFG;
		$this->try = false;
		...
		if(!empty($this->CFG['auth'])){ // Авторизация
			$auth = false;
			$sfile = 'ses.php';
			...
			if (!$auth) {
				if(!empty($_POST['ajax'])){
					print "sxd.hideLoading();alert('Session not found');";
					exit;
				}
	...
	function loadJob($job){
		$file = $this->CFG['backup_path'] . 'sj_' .$job['job'] . '.job.php';
	...
	function deleteDB($name){
		$r = mysql_query('DROP DATABASE `' . mysql_escape_string($name) . '`');
		if($r){
	...
	function addRestoreJob($job) {
		...
		foreach($objects AS $t => $list){
			if(empty($object[$t]) && empty($filter[$t])) continue;
	...
	function runRestoreJob($continue = false){
		...
		//mysql_query("SET NAMES 'UTF8'");
		$types = array('VI' => 'View', 'PR' => 'Procedure', 'FU' => 'Function', 'EV' => 'Event');
		$fcache = '';
		$writes = 0;
		$insert = '';
		$old_charset = '';
			...
			switch($q{0}){
				...
				case 'I':
					if (preg_match('/^INSERT( INTO `(.+?)` VALUES )/', $q, $m)) {
						...
						if($td != 'INSERT') $q = substr_replace($q, $td, 0, 6);
						//mysql_query("LOCK TABLES `{$tab}` WRITE") or die (mysql_error());
						mysql_query("ALTER TABLE `{$tab}` DISABLE KEYS") or die (mysql_error());
						if(!empty($this->JOB['autoinc'])) mysql_query("ALTER TABLE `{$tab}` AUTO_INCREMENT = 1") or die (mysql_error());
						$ex = 1;
					}
					break;
				case 'C':
				...
				case '#': // Команды для дампера
					if (preg_match("/\#\t(TC)`(.+?)`(([^_]+?)_.+?)$/", $q, $m)) {
						if(!empty($tab)) $enable_index[] = $tab;
						//if(!empty($tab)) mysql_query("ALTER TABLE `{$tab}` ENABLE KEYS") or die (mysql_error());
						$this->setNames($this->JOB['correct'] == 1 && !empty($this->JOB['charset']) ? $this->JOB['charset'] : $m[3]);
						$this->addLog(sprintf($this->LNG['restore_TC'], $m[2]));
						$insert = '';
						$tab = '';
						if($tc) if(mysql_query("{$tc} `{$m[2]}`")){}else {error_log("{$tc} `{$m[2]}`\n\n", 3, "error.log");};
					}
					$ei = preg_match("/^\#\tTD`(.+?)`/", $q) ? 1 : 0;
					$ex = 0;
					break;
				default: 
					$insert = '';
		...
		foreach($enable_index AS $tab){
			mysql_query("ALTER TABLE `{$tab}` ENABLE KEYS") or die (mysql_error());
			$this->rtl[4] = 'EK';
			$this->rtl[7] = 0;
			$this->rtl[8] = 0;
			fseek($this->fh_rtl, 0);
	...
	function addBackupJob($job) {
		...
		if (V_MYSQL > 50014 && (!empty($object['VI']) || !empty($filter['VI']))) {
			...
			$r = mysql_query("SELECT table_name, view_definition, collation_connection FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = '{$this->JOB['db']}'");
		...
		$this->JOB['file'] = sprintf('%s_%s.%s', $this->JOB['db'], date('Y-m-d_H-i-s'), $this->JOB['file_ext']);
	...
	function runBackupJob($continue = false){
		...
		mysql_query("SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') ."'");
		$time_old = time();
				...
				switch($n[0]){
					...
					case 'TA':
						...
						if ($n[0] == 'TC' || $this->rtl[7] == 0){
							...
							$r = mysql_query("SHOW CREATE TABLE `{$n[1]}`");
						...
						$notNum = array();
						$r = mysql_query("SHOW COLUMNS FROM `{$n[1]}`") or die(mysql_error());
			            $fields = 0;
			            ...
			            $z = 0;
			            // Достаем данные
			            $r = mysql_unbuffered_query("SELECT /*40101 SQL_NO_CACHE */ * FROM `{$n[1]}`{$from}");
					...
					case 'EV':
						...
						$r = mysql_query("SHOW CREATE {$types[$n[0]]} `{$n[1]}`");
						$item = mysql_fetch_assoc($r);
						$fcache .= "#\t{$n[0]}`{$n[1]}`{$n[2]}\t;\n" . preg_replace("/DEFINER=`.+?`@`.+?` /", '', $item['Create ' . $types[$n[0]]]) . "\t;\n";
					break;

					case 'TR':
						$this->addLog(sprintf($this->LNG['backup_TR'], $n[1]));
						if(V_MYSQL > 50120){
							// SHOW CREATE TRIGGER отсутствует до MySQL 5.1.21 
							$r = mysql_query("SHOW CREATE TRIGGER `{$n[1]}`");
							$item = mysql_fetch_assoc($r);
							$fcache .= "#\tTR`{$n[1]}`{$n[2]}\t;\n" . preg_replace("/DEFINER=`.+?`@`.+?` /", '', $item['SQL Original Statement']) . "\t;\n";
						}
						else {
							$r = mysql_query("SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'");
							$item = mysql_fetch_assoc($r);
		...
		if ($this->JOB['del_time'] || $this->JOB['del_count']) {
            ...
            if ($dh = opendir($this->CFG['backup_path'])) {
                ...
                while (false !== ($file = readdir($dh))) { 
                    if (preg_match("/^{$this->JOB['db']}_(\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})\.sql/", $file, $m)) { 
	...
	function setNames($collation){
		...
		if($this->rtl[6] != $collation) {
			mysql_query('SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'');
	...
	function getDBList(){
		...
		if(V_MYSQL < 50000){
			foreach($items AS $item){
    			$tables = mysql_query("SHOW TABLES FROM `{$item}`");
    			if ($tables) {
			...
			$result = mysql_query("SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`");
			while($item = mysql_fetch_row($result)){
    			if($item[0] == 'information_schema' || $item[0] == 'mysql') continue; 
	...
	function getCharsetList(){
		...
		if(V_MYSQL > 40101) {
    		...
    		$r = mysql_query("SHOW CHARACTER SET"); 
    		if ($r) {
    			while($item = mysql_fetch_assoc($r)){
	...
	function getFileObjects($tree, $name, $formatTree = true){
		...
		if(!preg_match('/^(#SXD20\|.+?)\n#EOH\n/s', $temp, $m)) return "sxd.tree.{$tree}.error(sxd.lng('err_sxd2'))";
		$head = explode("\n", $m[1]);
		$h = explode('|', $head[0]);
		...
		return $formatTree ? $this->formatTree($tree, $objects) .  "sxd.comment.restore.value = '{$h[9]}';" : $objects;
	}
...
function sxd_error_handler($errno, $errmsg, $filename, $linenum, $vars){
    ...
    if($errno == 8 || $errno == 1024) {
    	if (!$SXD->fh_log) echo isset($_POST['ajax']) ? "alert('" . mysql_escape_string($str) . "');" : $str;
    	else fwrite($SXD->fh_log, date('Y.m.d H:i:s') . "\t3\t{$str}\n");
	}
    elseif($errno < 1024) {
    	$SXD->error = true;
    	if (!$SXD->fh_log) echo isset($_POST['ajax']) ? "alert('" . mysql_escape_string($str) . "');" : $str;
...
<link rel="stylesheet" type="text/css" href="get.php?sxd.v203.css">
<script type="text/javascript" src="get.php?sxd.v203.js"></script>
<script type="text/javascript" src="get.php?{$SXD->LNG['name']}.lng.js"></script>
...
<div id="main_div">
	...
	<div id="content" class="content">
		...
		<table cellspacing="0" id="tab_restore" style="display:none;">
			...
			<tr><td></td><td align="right"><input type="button" value="{$SXD->LNG['btn_save']}" onclick="sxd.showDialog('savejob');"> <input type="button" value="{$SXD->LNG['btn_exec']}" onclick="sxd.runRestore();"></td></tr>
		</table>
		...
		<table cellspacing="0" id="tab_log" style="display:none;">
			...
			<tr><td width="242"><input type="button" value="{$SXD->LNG['btn_clear']}" onclick="sxd.log.clear();"></td><td width="290" align="right">
				...
				<legend>{$SXD->LNG['auth']}</legend>
					...
					<table cellspacing="3" id="hst" style="display:none;">
						<tr>
							...
							<td width="116"><input type="text" name="host" style="width:120px;" value="{$_POST['host']}"></td>
							<td width="40" align="right">{$SXD->LNG['my_port']}</td>
